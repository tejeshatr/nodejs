...
// Add the universal header and footer to a string and pass provided header and footer for interpolation
		...
		if(!err && headerString){
			...
			helpers.getTemplate('_footer', data, function(err, footerString){
				if(!err && footerString){
					...
					callback(err,'Could not find the footer template')
			});
		} else{
			callback(err,'Could not find the header template');
		}
	});
}
...
helpers.interpolate = function(str, data){
	...
	// For each key in data, insert its value into the string and corresponding placeholder
	for(let key in data){
	...
	if(fileName){
		...
		fs.readFile(publicDir+fileName, function(err, data){
			if(!err && data)
				...
				callback(err, 'No file was found');
		});
	} else
